...
class Builder
{
    ...
    {
        $logical = strtoupper(ArrayUtil::get($condition, 'logical', 'and'));

        if (ArrayUtil::get($condition, 'type') === 'nest') {
            /** @var Builder $query */
            $query = ArrayUtil::get($condition, 'query');
            if ($logical === 'OR') {
                $where->or;
            }
    ...
    {
        $not = ArrayUtil::get($condition, 'not', false) === true;
        $notChar = '';
        if ($not === true) {
